# File: googlethreatintelligence_connector.py
#
...
from googlethreatintelligence_consts import (
    GOOGLETHREATINTELLIGENCE_CONNECTIVITY_ENDPOINT,
    GOOGLETHREATINTELLIGENCE_ERR_CONNECTIVITY_TEST,
    GOOGLETHREATINTELLIGENCE_SUCC_CONNECTIVITY_TEST,
)


class GooglethreatintelligenceConnector(BaseConnector):
    """
    Googlethreatintelligence connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = GOOGLETHREATINTELLIGENCE_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(GOOGLETHREATINTELLIGENCE_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(GOOGLETHREATINTELLIGENCE_SUCC_CONNECTIVITY_TEST)
